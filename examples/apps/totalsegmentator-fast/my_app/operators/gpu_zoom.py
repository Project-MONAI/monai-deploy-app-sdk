...
def zoom_factors_for(in_shape: Sequence[int], out_shape: Sequence[int]) -> list:
    ...
    return [1.0 / (float(i) / float(o)) for i, o in zip(in_shape, out_shape)]
    ...
    *,
    ...
    if x.ndim != 3:
        raise NotImplementedError(
            "stock_gpu_zoom supports ndim=3 only (all active call sites are " f"3D); got ndim={x.ndim}"
        )
...
def _precompute_axes(
    ...
    starts = []
    weights = []
    for _axis, (n_in, n_out) in enumerate(zip(in_shape, out_shape)):
...
def _get_filter_kernel() -> cp.RawKernel:
    ...
    if _filter_kernel_cache is None:
        ...
        _filter_kernel_cache = cp.RawKernel(src, "spline_filter1d_kernel", options=_KERNEL_OPTIONS)
...
def _spline_prefilter(x64: cp.ndarray) -> cp.ndarray:
    ...
    for axis in range(3):
        ...
        if line_len > _MAX_LINE:
            raise NotImplementedError(f"axis {axis} length {line_len} exceeds the kernel line " f"buffer ({_MAX_LINE})")
...
def _get_kernel(in_type: str, out_type: str) -> cp.RawKernel:
    ...
    if key not in _kernel_cache:
        src = _KERNEL_TEMPLATE.replace("__OUT__", out_type).replace("__IN__", in_type)
        if out_type == "float":
            src = src.replace("out[oi] = (float)(t);", "out[oi] = __double2float_rn(t);")
        _kernel_cache[key] = cp.RawKernel(src, "gpu_zoom3_kernel", options=_KERNEL_OPTIONS)
...
def gpu_zoom_grid_mode(
    ...
    *,
    ...
    if x.ndim != 3:
        raise NotImplementedError(
            "gpu_zoom_grid_mode supports ndim=3 only (all active call sites " f"are 3D); got ndim={x.ndim}"
        )
    if not x.flags.c_contiguous:
    ...
    if output_dtype not in (cp.float32, cp.float64):
        raise ValueError(f"output_dtype must be cp.float32 or cp.float64, " f"got {output_dtype!r}")

    in_shape = tuple(int(s) for s in x.shape)
    if out_shape is None:
        out_shape = tuple(int(np.ceil(f * s)) for f, s in zip(zoom_factors, in_shape))
    else:
        out_shape = tuple(int(s) for s in out_shape)
        derived = tuple(int(np.ceil(f * s)) for f, s in zip(zoom_factors, in_shape))
    ...
    if weights:
        ...
        spl = cp.concatenate([cp.asarray(w, dtype=cp.float64).ravel() for w in weights])
    else:
        spl = cp.zeros(1, dtype=cp.float32)  # dummy (never dereferenced)
    starts_buf = cp.concatenate([cp.asarray(s, dtype=cp.int32) for s in starts])

    out = cp.empty(out_shape, dtype=output_dtype)
    kernel = _get_kernel(in_type, "float" if output_dtype == cp.float32 else "double")
    block = (8, 8, 8)
    grid = (
        (out_shape[0] + block[0] - 1) // block[0],
