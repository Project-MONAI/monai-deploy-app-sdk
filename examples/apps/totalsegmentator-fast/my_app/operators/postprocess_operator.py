...
try:  # package-style import (my_app.*)
    ...
    # NOTE (P7 deviation, minimal): sc_overlay.py is airway-only and is NOT
    # ported into totalsegmentator-fast. The two SC/contour helpers are
    # imported lazily inside the methods that use them (never called in
    # P7; if invoked they fail loudly with a clear error). This keeps the
    # operators package importable with the airway files absent.
...
def cc_label_gpu(mask: cp.ndarray) -> Tuple[cp.ndarray, int]:
    ...
    for _iteration in range(max_iters):
        ...
        if bool((labels == old).all()):
            ...
            return labels, num
    raise RuntimeError(
        f"cc_label_gpu did not converge within {max_iters} iterations " f"(mask shape {tuple(mask.shape)})"
    )
...
class PostprocessOperator(Operator):
    ...
    def _load_rules(self) -> List[Dict[str, Any]]:
        if self._rules is None:
            if self.model_path is None:
                ...
                self._logger.info("Loaded %d postprocessing rule(s) from postprocessing.pkl", len(self._rules))
        return self._rules
    ...
    def _write_sc_output(self, seg_cpu: np.ndarray, image: Image) -> Path:
        ...
        if self.output_folder is None:
            raise RuntimeError("PostprocessOperator requires output_folder to emit the 'dicom_sc_dir' output.")
        metadata = image.metadata()
        affine = np.asarray(metadata.get("nifti_affine_transform"))
        if affine.size == 0 or tuple(affine.shape) != (4, 4):
            raise ValueError(
                "Image metadata is missing a 4x4 'nifti_affine_transform'; " "cannot write the SC overlay."
            )
        image_volume = np.asarray(image.asnumpy())
        if image_volume.shape != seg_cpu.shape:
            raise ValueError(f"SC overlay shape mismatch: image {image_volume.shape} vs seg {seg_cpu.shape}.")
        sc_temp_dir = self.output_folder / "temp"
        try:  # lazy import: sc_overlay is airway-only (not ported into this app)
            from my_app.operators.sc_overlay import write_sc_overlay
        except ImportError:
            from operators.sc_overlay import write_sc_overlay
    ...
    def _voxel_volume_mm3(image: Image) -> float:
        ...
        if affine.size == 0 or tuple(affine.shape) != (4, 4):
            raise ValueError(
                "Image metadata is missing a 4x4 'nifti_affine_transform'; " "cannot compute the volume measurement."
            )
    ...
    def compute(self, op_input: Any, op_output: Any, context: Any) -> None:
        ...
        with nvtx_range("postprocess"):
            ...
            for label in self.output_labels:
                ...
                volume_lines.append(
                    f"{name.capitalize()} Volume: {calculate_volume_ml(seg_cpu, label, voxel_volume)} mL"
                )
            result_text = "\n".join(volume_lines)
            if not result_text:
            ...
            try:  # lazy import: sc_overlay is airway-only (not ported into this app)
                from my_app.operators.sc_overlay import reference_label_to_contour
            except ImportError:
                from operators.sc_overlay import reference_label_to_contour
