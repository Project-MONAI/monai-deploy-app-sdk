...
Phase 8 (pre-resample parity): ``preprocess_image`` first runs a
``pre_resample_to_model_spacing`` stage BEFORE the nnUNet chain — the
oracle's ``ResampleToModelSpacingd`` semantics (08-RESEARCH §1): integer-
dtype guard, RAS-ordered array, zoom factors = RAS affine column norms ÷
1.5 mm, the oracle's exact allclose skip window, and the oracle-identical
cubic cast chain (scipy CPU default; ``HOLOSCAN_GPU_PRERESAMPLE=1`` opts
into the byte-identical cupyx mirror). The emitted ``preprocessed_meta``
carries the orig metadata keys (``_pre_resample_spatial_shape``,
``orig_spacing_xyz``, ``pre_resample_skipped``, ``pre_resample_spacing``)
that Phase 9's back-resample (ResampleToOriginalSpacingd equivalent)
consumes; on the resample branch the chain's own resample degenerates to
a proven no-op (unit: .planning/scripts/p8_preprocess_noop_test.py).
...
try:  # package-style import (my_app.*)
    ...
    from my_app.operators.pre_resample_zoom import (
        TARGET_SPACING,
    ...
    from operators.gpu_zoom import (
        gpu_resample_enabled,
        stock_gpu_resize,
        stock_gpu_zoom,
        zoom_factors_for,
...
# 26-04: env gate for the synthetic-cascade dev path. Inlined (NOT imported
# from slidewindow_operator) to avoid an import cycle — slidewindow_operator
# imports to_holoscan_gpu_tensor from this module. Must stay in lockstep with
# slidewindow_operator.synthetic_cascade_enabled.
SYNTHETIC_CASCADE_ENV = "HOLOSCAN_SYNTHETIC_CASCADE"


def _synthetic_cascade_enabled() -> bool:
    """True iff HOLOSCAN_SYNTHETIC_CASCADE == "1" (strict; anything else off)."""
...
def _normalize_channel(
    image: np.ndarray, params: PreprocessParams, channel: int, mask: Optional[np.ndarray]
) -> np.ndarray:
...
def _resample_to_shape(
    ...
    """
    do_separate_z, axis = _determine_do_sep_z_and_axis(params.resample_force_separate_z, current_spacing, new_spacing)
...
def _resize_segmentation(segmentation: np.ndarray, new_shape: Sequence[int], order: int) -> np.ndarray:
    ...
    if gpu_resample_enabled() and segmentation.ndim == 3:
        ...
        if order == 0:
            ...
            m0 = cp.clip(m0, float(cp.min(base).get()), float(cp.max(base).get()))
    ...
    if np.any(shape != new_shape):
        ...
        if do_separate_z:
            ...
            for c in range(data.shape[0]):
                ...
                for slice_id in range(shape[axis]):
                    if axis == 0:
                        reshaped_here[slice_id] = _resize_segmentation(data[c, slice_id], new_shape_2d, order)
                    elif axis == 1:
                        reshaped_here[:, slice_id] = _resize_segmentation(data[c, :, slice_id], new_shape_2d, order)
                    else:
...
def reorient_to_ras(arr_dhw: np.ndarray, affine: np.ndarray) -> np.ndarray:
    """Re-orient the SDK's DHW array to canonical RAS (axis order + direction).

    The SDK to-volume path (affine_lps_to_ras=True) emits the array in (D, H, W)
    index order with a RAS-space affine whose columns can carry NEGATIVE
    in-plane direction signs (this corpus: L, P, S — measured in 08-04:
    col0=(-sp,0,0), col1=(0,-sp,0), col2=(0,0,+2.0)). The oracle corrects that
    with MONAI Orientationd(axcodes="RAS"), a pure FLIP of the affected axes
    (integer-exact, no interpolation). We reproduce it with np.flip along every
    axis whose affine column has a negative dominant sign — sign-driven, never
    hard-coded axes (08-04 recommended fix). The zoom factors are unaffected
    (column norms are sign-invariant).
    """
    out = arr_dhw.T  # (D, H, W) -> (W, H, D): axis i of `out` is world axis i
    cols = np.asarray(affine, dtype=np.float64)[:3, :3]
    dominant = np.argmax(np.abs(cols), axis=0)  # per column, which world axis it aligns with
    signs = np.sign(cols[dominant, np.arange(3)])  # direction sign per array axis
    for i in range(3):
        if signs[i] < 0:
            out = np.flip(out, axis=i)  # negative-stride view; no value copy
    return out
...
class PreprocessOperator(Operator):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            normalize: P9 (09-04) — apply per-channel intensity normalization
                (PREP-02) inside this operator. True (default) is the
                single-part P8 behavior. In the 5-part chain the shared
                preprocess sets this False and emits the resampled+cropped
                UNNORMALIZED volume; each SlideWindowOperator then applies its
                OWN part's CT properties (oracle parity — the oracle's shared
                ResampleToModelSpacingd also never normalizes; each part's
                run_case_npy normalizes with its own plans' properties).
            input_resample_order: pre-resample (ResampleToModelSpacingd
                replica) scipy interpolation order — default 3 = the
                oracle/MONAI byte-locked value (total); the single_model /
                multi_part branches pass TaskSpec.resample_order (body=1,
                TS 2.18 CLI parity; Phase 19, 19-03). Must be an explicit
                named parameter — a bare **kwarg would leak into the
                holoscan Operator base.
        """
        ...
        self._input_resample_order = int(input_resample_order)
    ...
    def setup(self, spec: OperatorSpec) -> None:
        ...
        if self.model_path is not None:
            ...
            self._load_params()
            if self._params.previous_stage is not None and not _synthetic_cascade_enabled():
    ...
    def _load_params(self) -> PreprocessParams:
        if self._params is None:
            if not self.model_path:
                raise RuntimeError("PreprocessOperator requires model_path to load plans.json parameters.")
            self._params = load_preprocess_params(self.model_path, self.config_name)
            self._logger.info(
                "Loaded preprocessing params for config %s: spacing=%s transpose_forward=%s " "normalization=%s",
    ...
    def _derive_spacing_xyz(image: Image) -> Tuple[float, float, float]:
        ...
        if affine is None or affine.size == 0 or tuple(affine.shape) != (4, 4):
            raise ValueError(
                "Image metadata is missing a 4x4 'nifti_affine_transform'; " "cannot derive voxel spacing."
            )
    ...
    def preprocess_image(
        ...
        # ------------------------------------------------------------------
        # 0. pre_resample_to_model_spacing (Phase 8, oracle parity —
        #    08-RESEARCH §1): BEFORE the nnUNet chain, bring the RAS-
        #    ordered volume to the model's 1.5 mm target spacing with the
        #    oracle's exact semantics — factors = RAS affine column norms
        #    ÷ 1.5, skip = the oracle's allclose window, resample = the
        #    oracle-identical cubic cast chain (scipy CPU default;
        #    HOLOSCAN_GPU_PRERESAMPLE=1 opts into the byte-identical
        #    cupyx mirror). A skipped volume passes through UNTOUCHED (the
        #    oracle's skip is a full `continue` — no cast, no copy).
        #    On the resample branch the chain's own resample then
        #    degenerates to a provable no-op (new_shape == current shape
        #    -> the 'No resampling necessary' early return; unit-proven in
        #    .planning/scripts/p8_preprocess_noop_test.py).
        # ------------------------------------------------------------------
        if arr.dtype.kind not in "iu":
            raise ValueError(
                f"pre_resample_to_model_spacing requires an integer input volume "
                f"(int16 expected for this corpus), got {arr.dtype} — the oracle's "
                f"quantization chain (scipy int16 round at write time) is undefined "
                f"for float input."
            )
        if lowres_seg is not None:
            # Cascade wiring is out of scope for the total task — no cascade
            # configs ship in models/total — so this guard stays as fail-fast.
            # (The cascade seg would arrive in the ORIGINAL orientation/shape
            # and silently mis-align against the pre-resampled image's crop
            # bbox; failing loudly keeps that class of bug visible.)
            raise NotImplementedError(
                "cascade + pre-resample wiring lands in Phase 9 — the lowres_seg "
                "input arrives in the ORIGINAL orientation/shape and cannot be "
                "cropped with the pre-resampled image's bbox."
            )
        affine = np.asarray(image.metadata().get("nifti_affine_transform"))
        zoom_factors = preresample_zoom_factors(affine)  # column norms / 1.5, RAS
        pre_resample_skipped = bool(preresample_should_skip(zoom_factors))
        # 08-04 fix: arr.T alone is NOT RAS — the SDK affine's in-plane column
        # signs are negative (L, P), so the oracle's Orientationd(RAS) flips
        # those axes before resampling. reorient_to_ras() reproduces that
        # integer-exact flip, sign-driven (never hard-coded axes).
        arr_ras = reorient_to_ras(arr, affine)
        # Orig RAS spatial shape, captured BEFORE the skip check — the oracle
        # stores _pre_resample_spatial_shape before its allclose `continue`
        # (:99-100), so the back-resample target exists on BOTH paths.
        orig_spatial_shape_ras = list(arr_ras.shape)
        if pre_resample_skipped:
            pass  # untouched: no cast, no value copy (flip is a view)
        else:
            arr_ras = preresample_volume(
                np.ascontiguousarray(arr_ras)[None].astype(arr.dtype, copy=False),
                zoom_factors,
                order=self._input_resample_order,
            )[
                0
            ]  # int32 out (oracle's line-130 cast chain)
        # The pixdim-equivalent the chain sees: orig on skip (the oracle keeps
        # orig pixdim on skip too), 1.5 mm on resample. This is what must
        # reach slidewindow (roadmap 8.2).
        pre_resample_spacing = list(spacing_xyz) if pre_resample_skipped else [float(s) for s in TARGET_SPACING]
        self._logger.info(
            "pre_resample: skipped=%s factors=%s in=%s out=%s order=%s",
            pre_resample_skipped,
            [round(float(f), 6) for f in zoom_factors],
            orig_spatial_shape_ras,
            list(arr_ras.shape),
            self._input_resample_order,
        )
        ...
        # ------------------------------------------------------------------
        raw = np.ascontiguousarray(arr_ras, dtype=arr_ras.dtype)
        ...
        # Reference stores properties["spacing"] in the reversed (z, y, x) order.
        # Phase 8: the chain sees the POST-pre-resample spacing (orig on skip,
        # 1.5 mm on resample) — on the resample branch new_shape == current
        # shape and the chain's resample is the proven no-op early return; on
        # the skip branch pre_resample_spacing == orig, so nothing changes.
        spacing = tuple(reversed(pre_resample_spacing))
        original_spacing = [spacing[i] for i in tf]

        # 2. crop, remember the size before cropping! (PREP-04)
        properties: Dict[str, Any] = {}
        # Phase 8 orig metadata — set on BOTH skip and resample paths (mirror
        # of the oracle, which stores the orig shape BEFORE the skip check).
        # Consumed by Phase 9's back-resample (ResampleToOriginalSpacingd
        # equivalent).
        properties["_pre_resample_spatial_shape"] = orig_spatial_shape_ras
        properties["orig_spacing_xyz"] = list(spacing_xyz)
        properties["pre_resample_skipped"] = pre_resample_skipped
        properties["pre_resample_spacing"] = list(pre_resample_spacing)
        ...
        vol_c[...] = vol_c_src
        properties["bbox_used_for_cropping"] = bbox
        properties["shape_after_cropping_and_before_resampling"] = tuple(int(s) for s in vol_c.shape[1:])
        ...
        if lowres_seg is not None:
            seg4 = cp.array(
                lowres_seg
            )  # uint8 G2G copy via __cuda_array_interface__ (NOT cp.from_dlpack — ownership pitfall)
        ...
        #    resampling (reference order) WHEN IT HAPPENS AT ALL. P9 (09-04):
        #    ``normalize=False`` skips this step entirely and emits the
        #    resampled+cropped volume raw — the 5-part chain's SlideWindow
        #    operators each apply their own part's CT properties downstream
        #    (oracle parity: normalization is per-part there, so a shared
        #    single-property normalization upstream is wrong for 4 of 5 parts).
        if self._normalize:
            # The mean/std REDUCTIONS stay on the numpy reference path: CuPy
            # reductions are not bit-identical to numpy (reduction order
            # differs — RESEARCH Pitfall 4); the element-wise subtract/divide
            # (and clip) are bit-identical in fp32.
            self._normalize_channels(vol_c, vol_c.get(), nonzero_mask, slicer, params)
        # Materialize after per-channel writes (D-12) before leaving the GPU.
        # (No-op copy when normalize=False — the 5-part chain emits the
        # resampled+cropped UNNORMALIZED volume and each SlideWindowOperator
        # normalizes with its own part's CT properties downstream.)
        ...
        if gpu_resample_enabled():
            vol_out = _resample_to_shape(vol_c, new_shape, original_spacing, params.spacing, params)
        else:
            ...
            vol_out = _resample_to_shape(vol_out, new_shape, original_spacing, params.spacing, params)
        ...
        # ------------------------------------------------------------------
        seg_r = _resample_seg_to_shape(seg_c, new_shape, original_spacing, params.spacing, params)
        ...
        one_hot = self._buf_cache.get((len(params.foreground_labels), *seg3.shape), cp.float32)
        ...
        vol2 = self._buf_cache.get((vol_gpu.shape[0] + one_hot.shape[0], *seg3.shape), cp.float32)
    ...
    def _normalize_channels(
        self,
        vol_c,  # cp.ndarray (C, X, Y, Z) fp32, normalized in place
        vol_np,  # np.ndarray mirror (D2H) — numpy reduction path
        nonzero_mask,  # np.ndarray bool mask, pre-crop
        slicer,  # tuple[slice, ...] crop box
        params: PreprocessParams,
    ) -> None:
        """P9 (09-04): per-channel normalization extracted verbatim from
        ``preprocess_image`` step 3 (single-part P8 path, byte-identical)."""
        eps = 1e-8
        norm_mask = None
        if any(params.use_mask_for_norm):
        ...
        for c in range(vol_c.shape[0]):
            ...
            if scheme == "ZScoreNormalization":
                if params.use_mask_for_norm[c] and norm_mask is not None:
                    ...
                    m_gpu = cp.asarray(m)
                    ch[m_gpu] = (ch[m_gpu] - cp.asarray(mean)) / cp.asarray(np.float32(max(std, eps)))
                else:
                    mean = np.float32(ch_np.mean())
                    std = np.float32(ch_np.std())
                    vol_c[c] = (ch - cp.asarray(mean)) / cp.asarray(np.float32(max(std, eps)))
                ...
                vol_c[c] = cp.clip(ch, props["percentile_00_5"], props["percentile_99_5"])
                ...
                raise ValueError(f"unsupported normalization scheme {scheme!r} for channel {c}")
        # NOTE (09-OOM fix): the step 4 resample and the step 5 cascade
        # return live in preprocess_image — this method is the pure
        # in-place per-channel normalization (the f6b4bdf extraction had
        # stranded the resample+returns inside it, so preprocess_image
        # fell off its end and returned None on EVERY flag combination).
    ...
    def compute(self, op_input: Any, op_output: Any, context: Any) -> None:
        ...
        with nvtx_range(f"preprocess_{self.config_name}"):
            ...
            # 26-04: the synthetic-cascade dev path (HOLOSCAN_SYNTHETIC_CASCADE=1)
            # does NOT declare the port — SlideWindowOperator feeds the zero
            # one-hot stub itself from the 1-channel volume.
            lowres_seg: Optional[torch.Tensor] = None
            params = self._load_params()
            if params.previous_stage is not None and not _synthetic_cascade_enabled():
