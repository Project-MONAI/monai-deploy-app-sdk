...
try:  # package-style import (my_app.* on sys.path)
    from my_app.task_specs import TASK_REGISTRY
except ImportError:  # flat import (my_app dir on sys.path, as the app runner provides)
    from task_specs import TASK_REGISTRY
...
__all__ = [
    ...
    "resolve_use_mirroring",
    "resolve_mirror_flag",
    "MODEL_PARTS",
    "EXPECTED_MAX_LOCAL_LABEL",
    "resolve_total_model_root",
...
def find_jsonpkls_dir(model_path: ModelPath) -> Path:
    ...
    raise FileNotFoundError(
        f"jsonpkls/plans.json not found under {model_path} (tried {base}/jsonpkls and " f"{base}/models/jsonpkls)."
    )
...
class InferenceParams:
    ...
    # D-04 (26-01): plan-driven TTA mirroring flag.
    # Resolution order: configurations[<cfg>].use_mirroring > top-level
    # plans.use_mirroring > False. No shipped plans.json (0/22 in corpus) sets
    # the key, so every registry bundle resolves to False (behavior-neutral).
    use_mirroring: bool
    checkpoint_name: str
    folds: Tuple[int, ...]
...
def resolve_use_mirroring(plans: Dict[str, Any], config_name: str) -> bool:
    """D-04 (26-01): resolve the TTA mirroring flag from the bundle plans.

    Precedence (26-01-PLAN interfaces, EXACT names for 26-03/26-04):
      1. ``plans["configurations"][config_name]["use_mirroring"]`` if the
         per-config key is present (per-config wins over top-level);
      2. ``plans["use_mirroring"]`` (top-level fallback) if that key is present;
      3. ``False`` — today's behavior (no shipped plans.json sets the key: 0/22
         in the corpus, so every registry bundle resolves to False and D-04 is
         behavior-neutral until a plan opts in).

    Pure function: never raises on malformed plans (missing "configurations",
    non-dict config entry, unknown config name) — those all fall through to
    the top-level key or False.
    """
    configurations = plans.get("configurations") or {}
    cfg_entry = configurations.get(config_name)
    if isinstance(cfg_entry, dict) and "use_mirroring" in cfg_entry:
        return bool(cfg_entry["use_mirroring"])
    if "use_mirroring" in plans:
        return bool(plans["use_mirroring"])
    return False


def resolve_mirror_flag(explicit: Optional[bool], plan_value: bool) -> bool:
    """D-04 (26-01): operator-constructor precedence — an explicit True/False
    ``use_mirroring`` argument beats the plan value; ``None`` defers to the
    plan-driven value from :func:`resolve_use_mirroring`. Pure function.
    """
    return plan_value if explicit is None else bool(explicit)
...
def resolve_checkpoint_name(config_dir: Union[str, Path], checkpoint_name: Optional[str] = None) -> str:
    ...
    available = sorted({p.name for p in config_dir.glob("*.pt")} | {p.name for p in config_dir.glob("fold_*/*.pt")})
...
def detect_available_folds(config_dir: Union[str, Path], checkpoint_name: str) -> List[int]:
    ...
    config_dir = Path(config_dir)
    return sorted(
        int(d.name.split("_")[-1]) for d in config_dir.glob("fold_*") if d.is_dir() and (d / checkpoint_name).exists()
    )


# ---------------------------------------------------------------------------
# TotalSegmentator "total" model plumbing (Phase 7, plan 07-02)
#
# The "total" bundle is 5 single-task nnUNet models that together cover the
# 117-label TotalSegmentator label space. Each part is an independent bundle
# with its own jsonpkls/ + 3d_fullres/ under models/total/<part>/.
# ---------------------------------------------------------------------------

# Phase 17 (17-02): the task registry (my_app/task_specs) is the source of
# truth for task-level data. The "total" entry encodes the 5-part chain:
# organs (local 1-24 -> unified 1-24, offset 0), vertebrae (1-26 -> 25-50,
# offset 24), cardiac (1-18 -> 51-68, offset 50), muscles (1-23 -> 69-91,
# offset 68), ribs (1-26 -> 92-117, offset 91) -> 117 unified labels
# (offsets verified vs ct-totalsegmentator-map/app_total/nnunet_seg_operator.py:68-73).
# Derivations below keep the dict-of-str-keys shape that resolve_total_model_root,
# resolve_active_parts, and the operators index by (p["name"] / p["label_offset"]).
MODEL_PARTS: List[Dict[str, int]] = [
    {"name": p.name, "label_offset": p.label_offset} for p in TASK_REGISTRY["total"].parts
]

# Per-part max LOCAL label (= num foreground heads, verified against each
# part's dataset.json label table in the "total" bundle). Derivation over the
# registry — single source of truth; import from here, do NOT copy into
# operator modules.
EXPECTED_MAX_LOCAL_LABEL: Dict[str, int] = {p.name: p.max_local_label for p in TASK_REGISTRY["total"].parts}

# Files each part directory must contain for a "total" root to be valid.
# NOTE (17-02): the "3d_fullres" literal is a BUNDLE-LAYOUT constant (part of
# the on-disk "total" root contract), NOT task-selection data — it
# intentionally stays here rather than moving to the registry.
# NOTE (26-01, CFG02-01): the checkpoint requirement is ANY-fold — at least
# one `fold_*/` under `3d_fullres/` holding one of DEFAULT_CHECKPOINT_ORDER
# (pre-26-01 this was the literal `3d_fullres/fold_0/final_model.pt`).
_TOTAL_REQUIRED_FILES: Tuple[str, ...] = ("jsonpkls/plans.json",)

# Human-readable description of the any-fold checkpoint requirement, used in
# fail-fast error messages (the actual check is _has_any_fold_checkpoint).
_CHECKPOINT_REQ = f"fold_*/({' or '.join(DEFAULT_CHECKPOINT_ORDER)})"


def _has_any_fold_checkpoint(root: Path, config_name: str) -> bool:
    """CFG02-01 (26-01): True iff at least one `fold_*/` under
    `{root}/{config_name}/` contains one of DEFAULT_CHECKPOINT_ORDER.

    Replaces the pre-26-01 fold_0/final_model.pt literal assumption in both
    resolvers: a valid bundle whose checkpoint lives in a non-zero fold (or
    under a different supported checkpoint name) now resolves, matching what
    detect_available_folds + load_inference_params (fold-count-generic) will
    load. `jsonpkls/plans.json` remains a separate, still-required file.
    """
    ...
    for fold_dir in sorted(config_dir.glob("fold_*")):
        if fold_dir.is_dir():
            for ckpt in DEFAULT_CHECKPOINT_ORDER:
                if (fold_dir / ckpt).is_file():
                    return True
    return False


def _required_files_for_part(name: str, part_configs: Mapping[str, str]) -> Tuple[str, ...]:
    """Per-part required-file paths for resolve_task_model_root (22-02, 26-01).

    `jsonpkls/plans.json` + the any-fold checkpoint requirement under
    `{config_name}/` (CFG02-01: at least one `fold_*/` with one of
    DEFAULT_CHECKPOINT_ORDER, not the old fold_0/final_model.pt literal).
    config_name comes from the caller's mapping (spec-driven); a missing key
    raises ValueError (CFG02-02, 26-01 — the pre-26-01 silent `3d_fullres`
    fallback is gone; the 3d_fullres default for legacy callers that omit the
    mapping entirely now lives in resolve_task_model_root itself).

    Raises:
        ValueError: when `name` is absent from `part_configs`.
    """
    if name not in part_configs:
        raise ValueError(
            f"_required_files_for_part: part {name!r} missing from part_configs mapping "
            f"(available: {sorted(part_configs)}; no silent 3d_fullres fallback)"
        )
    config_name = part_configs[name]
    return (
        "jsonpkls/plans.json",
        f"{config_name}/{_CHECKPOINT_REQ}",
    )


def resolve_total_model_root(model_path: ModelPath) -> Path:
    """Resolve the root directory of a complete TotalSegmentator "total" bundle.

    Tries, in order (first hit wins):
      1. ``model_path`` itself
      2. ``model_path / "models"``
      3. ``model_path / "models" / "total"``
      4. ``model_path / "total"``

    A candidate directory "hits" when it looks like a total root, i.e. it
    contains at least one part directory from ``MODEL_PARTS``. The first
    such candidate must be COMPLETE: all 5 part directories present, each
    with ``jsonpkls/plans.json`` AND at least one ``fold_*/`` under
    ``3d_fullres/`` holding a supported checkpoint (CFG02-01, 26-01; pre-
    26-01 this was the literal ``3d_fullres/fold_0/final_model.pt``).

    There is deliberately NO partial-availability fallback (the oracle's
    ``_detect_model_parts`` silently degrades to the available parts —
    that anti-feature is not ported): a hit candidate missing any part or
    file is a hard ``FileNotFoundError`` naming every missing file.
    Candidate directories that contain no part directories at all (e.g. a
    repo root or a MAP bake parent) are skipped, not raised on, so the
    ``models/total`` and ``total`` MAP-bake layouts resolve correctly.

    Args:
        model_path: str or Path; the app's ``-m`` argument (or a parent of it).

    Returns:
        The resolved total model root as a ``Path``.

    Raises:
        FileNotFoundError: if no candidate is complete, or no candidate dir
            exists at all.
    """
    p = Path(model_path)
    for root in (p, p / "models", p / "models" / "total", p / "total"):
        if not root.is_dir():
            continue
        has_any_part = any((root / part["name"]).is_dir() for part in MODEL_PARTS)
        if not has_any_part:
            continue
        missing = [
            f"{part['name']}/{rel}"
            for part in MODEL_PARTS
            for rel in _TOTAL_REQUIRED_FILES
            if not (root / part["name"] / rel).is_file()
        ]
        for part in MODEL_PARTS:
            if not _has_any_fold_checkpoint(root / part["name"], "3d_fullres"):
                missing.append(f"{part['name']}/3d_fullres/{_CHECKPOINT_REQ}")
        if not missing:
            return root
        raise FileNotFoundError(f"total model root invalid at {root}: missing {missing}")
    raise FileNotFoundError(f"no valid total model root under {model_path}")


def resolve_task_model_root(
    model_path: ModelPath,
    task_name: str,
    part_names: Sequence[str],
    part_configs: Optional[Mapping[str, str]] = None,
) -> Path:
    """Resolve the root directory of a complete model bundle for a task (18-01).

    Spec-driven generalization of ``resolve_total_model_root`` (LIV-04):
    the part set comes from the resolved TaskSpec instead of the hard-coded
    ``MODEL_PARTS``, so the same 4-layout contract works for every task.

    Tries, in order (first hit wins):
      1. ``model_path`` itself
      2. ``model_path / "models"``
      3. ``model_path / "models" / <task_name>``
      4. ``model_path / <task_name>``

    A candidate directory "hits" when it contains at least one part directory
    from ``part_names`` (same hit rule as the total resolver). The first such
    candidate must be COMPLETE: every part directory present, each with
    ``jsonpkls/plans.json`` AND at least one ``fold_*/`` under
    ``{config_name}/`` holding one of DEFAULT_CHECKPOINT_ORDER (CFG02-01,
    26-01; pre-26-01 this was the literal ``{config_name}/fold_0/
    final_model.pt``) where config_name is that part's ``PartSpec.config_name``
    (22-02: passed via ``part_configs``; CFG02-02, 26-01: an explicitly passed
    mapping missing a part's key raises ValueError from
    ``_required_files_for_part`` — no silent ``3d_fullres`` fallback. Passing
    ``part_configs=None`` (legacy callers, none in-app) defaults every part
    to ``3d_fullres`` — the layout of every pre-Phase-22 bundle). There is
    deliberately NO partial-availability fallback:
    a hit candidate missing any part or file is a hard ``FileNotFoundError``
    naming every missing file (LIV-04). Candidate directories that contain no
    part directories at all (e.g. a repo root or a MAP bake parent) are
    skipped, not raised on.

    For ``task_name == "total"`` with the ``MODEL_PARTS`` names this returns
    the exact same root ``resolve_total_model_root`` returns for every layout
    (the total resolver itself is left byte-identical; test_liver_specs.py
    asserts path equality on the real root).

    Args:
        model_path: str or Path; the app's ``-m`` argument (or a parent of it).
        task_name: registry task name (used only for the layout candidates).
        part_names: ordered part directory names from the resolved TaskSpec.
        part_configs: optional ``{part name: config_name}`` mapping from the
            resolved TaskSpec's ``PartSpec.config_name`` values; ``None`` (or a
            missing key) falls back to ``3d_fullres`` for that part.

    Returns:
        The resolved model root as a ``Path``.
    ...
    if part_configs is None:
        # Legacy callers that omit the mapping entirely keep the pre-22-02
        # layout (3d_fullres for every part). Explicit mappings are strict
        # (CFG02-02, 26-01: a missing key raises instead of falling back).
        configs = dict.fromkeys(part_names, "3d_fullres")
    else:
        configs = dict(part_configs)
    for root in (p, p / "models", p / "models" / task_name, p / task_name):
        if not root.is_dir():
            continue
        has_any_part = any((root / name).is_dir() for name in part_names)
        if not has_any_part:
            continue
        missing = [
            f"{name}/jsonpkls/plans.json"
            for name in part_names
            if not (root / name / "jsonpkls" / "plans.json").is_file()
        ]
        for name in part_names:
            if not _has_any_fold_checkpoint(root / name, configs[name]):
                missing.append(f"{name}/{configs[name]}/{_CHECKPOINT_REQ}")
        if not missing:
            return root
        raise FileNotFoundError(f"model root for task {task_name!r} invalid at {root}: missing {missing}")
    raise FileNotFoundError(f"no valid model root for task {task_name!r} under {model_path}")


def resolve_active_parts() -> List[Dict[str, int]]:
    """Resolve which model parts to load, from the HOLOSCAN_MODEL_PARTS env var.

    * Unset or empty -> the full ``MODEL_PARTS`` list (all 5, oracle order).
    * Comma-separated subset (e.g. ``"organs,ribs"``) -> the matching entries
      in ``MODEL_PARTS``, preserving the original oracle order (organs,
      vertebrae, cardiac, muscles, ribs) regardless of the order typed.

    Raises:
        ValueError: if any requested name is not a known part.
    """
    raw = os.environ.get("HOLOSCAN_MODEL_PARTS", "")
    if not raw.strip():
        return list(MODEL_PARTS)
    requested = {name.strip() for name in raw.split(",") if name.strip()}
    valid_names = {p["name"] for p in MODEL_PARTS}
    for name in sorted(requested):
        if name not in valid_names:
            raise ValueError(f"Unknown model part {name!r}. Valid parts: {sorted(valid_names)}")
    return [p for p in MODEL_PARTS if p["name"] in requested]


def _resolve_config_dir_and_jsonpkls(model_path: Union[str, Path], config_name: str) -> Tuple[Path, Path]:
...
def load_inference_params(
    ...
    if not folds:
        raise FileNotFoundError(f"No fold_* directories with {checkpoint_name!r} found under {config_dir}.")

    num_input_channels = determine_num_input_channels(plans_manager, configuration_manager, dataset_json)
    num_heads = plans_manager.get_label_manager(dataset_json).num_segmentation_heads

    return InferenceParams(
        use_mirroring=resolve_use_mirroring(plans, config_name),
        config_name=config_name,
