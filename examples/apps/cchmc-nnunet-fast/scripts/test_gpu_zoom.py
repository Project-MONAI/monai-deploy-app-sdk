...
import gpu_bootstrap  # noqa: E402, F401  (RMM first — shipping import order)
import numpy as np  # noqa: E402
from operators.gpu_zoom import gpu_zoom_grid_mode, zoom_factors_for  # noqa: E402
...
def ref_data_chain(x: np.ndarray, new_shape, order: int) -> np.ndarray:
    ...
    return sk_resize(x, tuple(int(s) for s in new_shape), order, mode="edge", anti_aliasing=False)
...
def gpu_seg_multihot(segmentation: np.ndarray, new_shape, order: int) -> np.ndarray:
    ...
    for c in np.unique(segmentation.ravel()):
        ...
        m = gpu_zoom_grid_mode(mask, zf, order, out_shape=new_shape, output_dtype=cp.float64).get()
        reshaped[m >= 0.5] = c
    return reshaped
...
def cupy_o0_sanity(x: np.ndarray, new_shape, name: str) -> None:
    ...
    grid3 = np.stack(
        np.meshgrid(
            *[np.broadcast_to(g, new_shape).ravel() for g in [np.arange(s, dtype=np.float64) for s in new_shape]]
        ),
        axis=-1,
    )
    ...
    got = cndi.map_coordinates(np.ascontiguousarray(x), coord_map, order=0, mode="nearest").astype(np.float32)
    del grid3, grid
    check(
        f"{name} [order-0 stock-CuPy cross-check]",
        ref_scipy_zoom(np.ascontiguousarray(x, dtype=np.float32), new_shape, 0),
        got,
    )
...
def main() -> int:
    ...
    check(
        "REAL(a) 256^3->(255,256,255) o3 primitive (scipy zoom fp32)",
        ref_scipy_zoom(xa[0], new_a, 3),
        np.ascontiguousarray(
            gpu_zoom_grid_mode(
                cp.asarray(np.ascontiguousarray(xa[0])),
                zoom_factors_for(xa[0].shape, new_a),
                3,
                out_shape=new_a,
            ).get()
        ),
    )
    print(f"  [timing] case (a) kernel+D2H per call: {t_kernel_a:.1f} ms " f"(gate: none — expected << 1 s)")

    # (b) 256^3 -> (201,202,201) order 3 — preprocess image (lowres)
    xb = rng.uniform(-3.0, 4.0, size=(1, 256, 256, 256)).astype(np.float32)
    new_b = (201, 202, 201)
    check(
        "REAL(b) 256^3->(201,202,201) o3 full chain",
        ref_data_chain(xb, new_b, 3),
        gpu_data_chain(xb, new_b, 3),
    )
    check(
        "REAL(b) 256^3->(201,202,201) o3 primitive (scipy zoom fp32)",
        ref_scipy_zoom(xb[0], new_b, 3),
        np.ascontiguousarray(
            gpu_zoom_grid_mode(
                cp.asarray(np.ascontiguousarray(xb[0])),
                zoom_factors_for(xb[0].shape, new_b),
                3,
                out_shape=new_b,
            ).get()
        ),
    ...
    check(
        "REAL(c) 256^3->(255,256,255) o1 multihot fp64 primitive (scipy zoom)",
        ref_scipy_zoom(mask64, new_c, 1),
        gpu_zoom_grid_mode(
            cp.asarray(np.ascontiguousarray(mask64, dtype=np.float32)),
            zoom_factors_for(mask64.shape, new_c),
            1,
            out_shape=new_c,
    ...
    new_d = (256, 256, 256)
    check(
        "REAL(d) (255,256,255)->256^3 o1 2ch full chain",
        ref_data_chain(xd, new_d, 1),
        gpu_data_chain(xd, new_d, 1),
    )
    for ci in range(2):
        check(
            f"REAL(d) ch{ci} primitive (scipy zoom fp32)",
            ref_scipy_zoom(xd[ci], new_d, 1),
            np.ascontiguousarray(
                gpu_zoom_grid_mode(
                    cp.asarray(np.ascontiguousarray(xd[ci])),
                    zoom_factors_for(xd[ci].shape, new_d),
                    1,
                    out_shape=new_d,
                ).get()
            ),
    ...
    for i, order in enumerate(orders):
        ...
        out_shape = tuple(max(16, min(340, int(round(s * sc)))) for s, sc in zip(in_shape, scales))
        x = rng.uniform(-1.5, 2.5, size=(1, *in_shape)).astype(np.float32)
        check(
            f"RANDOM[{i}] {in_shape}->{out_shape} o{order} full chain",
            ref_data_chain(x, out_shape, order),
            gpu_data_chain(x, out_shape, order),
        )
        check(
            f"RANDOM[{i}] {in_shape}->{out_shape} o{order} primitive (scipy zoom)",
            ref_scipy_zoom(x[0], out_shape, order),
            np.ascontiguousarray(
                gpu_zoom_grid_mode(
                    cp.asarray(np.ascontiguousarray(x[0])),
                    zoom_factors_for(in_shape, out_shape),
                    order,
                    out_shape=out_shape,
                ).get()
            ),
    ...
    for order in (1, 3):
        check(
            f"EDGE(e) 200^3->200^3 (zoom 1.0) o{order} full chain",
    ...
    new_f = (200, 300, 130)
    check(
        "EDGE(f) (257,259,263)->(200,300,130) o3 full chain",
        ref_data_chain(xf, new_f, 3),
        gpu_data_chain(xf, new_f, 3),
    )
    check(
        "EDGE(f) (257,259,263)->(200,300,130) o1 full chain",
        ref_data_chain(xf, new_f, 1),
        gpu_data_chain(xf, new_f, 1),
    )
    ...
    if FAILURES:
        print(
            f"RESULT: FAIL — {len(FAILURES)} case(s) diverge (D-22 fallback: "
            f"flag stays OFF, divergence recorded in evidence/gpu_resample_verdict.md)"
        )
        return 1
    print("RESULT: PASS — every case byte-identical (np.array_equal) vs the " "scipy/skimage reference chain")
    return 0
