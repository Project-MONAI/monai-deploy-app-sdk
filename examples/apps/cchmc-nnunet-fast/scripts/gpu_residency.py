...
ALLOWED_TRANSFER_FILES = {
    ...
    "postprocess_operator.py": "EXACTLY-ONCE boundary transfer seg_gpu.cpu().numpy() " "at the final output stage",
...
def run_static(app_root: Path) -> int:
    ...
    for fname in SCAN_FILES:
        ...
        for lineno, funcname, attr in calls:
            ...
            if not allowed:
                violations.append(f"{fname}:{lineno}: .{attr}() call in {funcname} is not " f"an allowed transfer site")
        ...
        if fname in OPERATOR_FILES:
            if not _compute_has_gpu_guard(tree):
                ...
                print("      compute() assert_on_gpu guard: MISSING -> VIOLATION")
            else:
                print("      compute() assert_on_gpu guard: present")
    ...
    if pp_path.is_file():
        ...
        if len(pp_cpu) != 1:
            violations.append(
                f"postprocess_operator.py: expected exactly 1 .cpu() boundary " f"transfer, found {len(pp_cpu)}"
            )
        print(
            f"  postprocess boundary .cpu() transfer count: {len(pp_cpu)} "
...
class TransferRecorder:
    ...
    def boundary(self):
        return [r for r in self.records if r["file"] == "postprocess_operator.py" and r["method"] == "cpu"]

    def resample(self):
        return [r for r in self.records if r["file"] == "postresample_operator.py"]


def _run_app_e2e(app_root: Path, input_dir: Path, model_dir: Path, output_dir: Path) -> None:
...
def _check_runtime(recorder: TransferRecorder, output_dir: Path):
    ...
    for r in recorder.records:
        ...
        print(f"      .{r['method']}() {r['file']}:{r['line']} in {r['func']} " f"(from {r['device']})  ->  {mark}")
    ...
    if not recorder.records:
        msgs.append("no app-attributed transfers recorded — the app chain did " "not run (or the hooks were bypassed)")
    if unexpected:
        for r in unexpected:
            msgs.append(
                f"UNEXPECTED CPU transfer: .{r['method']}() at "
                f"{r['file']}:{r['line']} in {r['func']} "
                f"(premature transfer before the final stage)"
            )
    if len(boundary) != 1:
        msgs.append(f"expected exactly 1 postprocess boundary .cpu() transfer, " f"saw {len(boundary)}")
    if not resample:
        msgs.append("no resample-path transfer recorded (postresample CPU path " "was not exercised)")
    if not seg_files:
        msgs.append("no DICOM-SEG written to the output dir")
    return (not msgs), msgs


def run_runtime(app_root: Path, input_dir: Path, model_dir: Path, output_dir: Path) -> int:
    print("=" * 72)
...
def run_self_test(app_root: Path, input_dir: Path, model_dir: Path, output_dir: Path) -> int:
    print("=" * 72)
    ...
    try:
        try:
            ...
            ok, msgs = False, [str(e)]
        unexpected = recorder.unexpected()
        detected = any(r["file"] == "slidewindow_operator.py" for r in unexpected)
        print(f"  injected violation detected by the detector: {detected}")
        print(f"  runtime check verdict on the injected run: " f"{'PASS' if ok else 'FAIL'} (FAIL expected)")
        if detected and not ok:
            print("  RESULT: PASS — the test correctly fails on an injected " "intermediate .cpu() call")
            rc = 0
        else:
            print("  RESULT: FAIL — the detector missed the injected violation " "or did not fail the run")
    finally:
        recorder.uninstall()
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--static", action="store_true", help="run the AST static scan (no GPU needed)")
    ap.add_argument(
        "--runtime",
        action="store_true",
    ...
    ap.add_argument(
        "--self-test",
        action="store_true",
        help="inject an illegal .cpu() and verify the detector " "flags it (GPU needed)",
    )
    ap.add_argument(
        "--input",
        default=str(REPO_ROOT / "testdata" / "airway_input"),
        help="DICOM study dir (default: testdata/airway_input)",
    )
    ap.add_argument(
        "--model",
        default=str(REPO_ROOT / "examples" / "apps" / "cchmc_nnunet_fifteen_ckpt_app" / "models"),
        help="nnUNet bundle dir (default: reference app models)",
    )
    ap.add_argument(
        "--output",
        default="/tmp/gpu_residency_out",
    ...
    if do_self:
        rc |= run_self_test(app_root, Path(args.input), Path(args.model), Path(args.output))
    elif do_runtime:
        rc |= run_runtime(app_root, Path(args.input), Path(args.model), Path(args.output))
    return rc
