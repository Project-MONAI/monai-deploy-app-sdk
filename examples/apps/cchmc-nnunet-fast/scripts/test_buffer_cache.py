...
def main():
    ...
    check(
        "torch: both shapes retained (s' does not evict s)",
    ...
    check(
        "torch: same shape, fp32 vs fp64 -> different buffers",
    ...
    check(
        "cupy: fp32 vs fp64 vs uint8 -> 3 distinct buffers",
        len({b32.data.ptr, b64.data.ptr, bu8.data.ptr}) == 3,
    )
    check(
        "cupy: all C-contiguous",
    ...
    check(
        "torch: zero=True borrow -> all zeros (prior contents NOT relied on)",
        torch.all(zb == 0),
    )
    zb.fill_(7.5)
    nz = tc.get(s_a, torch.float32, zero=False)
    check(
        "torch: zero=False borrow -> prior contents survive " "(documented; callers must overwrite before reading)",
        torch.all(nz == 7.5),
    )
    ...
    check("cupy: zero=False borrow -> prior contents survive", bool(cp.all(nzc == 4.5)))
    ...
    check(
        "torch: post-clear get() -> valid working buffer",
    ...
    check(
        "cupy: post-clear get() -> valid working buffer",
    ...
    check(
        "torch: total_bytes == sum over entries",
        tb.total_bytes() == expected,
        f"{tb.total_bytes()} vs {expected}",
    )
    check(
        "torch: keys() == (shape, str(dtype)) pairs",
    ...
    check(
        "shares_storage: offset VIEW of the buffer detected (data_ptr alone would miss it)",
    ...
    check(
        "cupy: shares_storage base + offset view",
    ...
    for fold in range(3):  # 3 fake folds
        ...
        if prev_ptr is not None:
            check(
                f"fold {fold + 1}: predicted_logits reuses study buffer " f"(same data_ptr)",
                logits.data_ptr() == prev_ptr,
            )
        check(
            f"fold {fold + 1}: zero-on-borrow -> all zeros after prior " f"fold's garbage",
            bool(torch.all(logits == 0)) and bool(torch.all(counts == 0)),
        )
    ...
    for fold in range(3):
        ...
        check(
            f"fold {fold + 1}: gaussian tensor UNMODIFIED by the loop " f"(read-only reuse)",
            bool(torch.equal(g, gaussian)),
        )
    check(
        "gaussian-once: 3 fake folds reused one tensor (identical data_ptr)",
        len(set(fold_ptrs)) == 1,
        f"ptrs={fold_ptrs}",
    )
    ...
    # sum is corrupted.
    n_folds = 3
    fold_vals = [torch.full(s_a, float(k + 1), device="cuda") * (0.5**k) for k in range(n_folds)]
    ...
    acc = acc / n_folds
    check(
        "multi-fold accumulation (cache + fold-1 clone) == fresh-allocation " "reference",
        bool(torch.equal(acc, ref)),
    )
    acc_buggy = None
    for k in range(n_folds):
        fl = swc.get(logit_shape, torch.float32, zero=True)
    ...
    check(
        "regression guard: WITHOUT the fold-1 clone the sum IS corrupted " "(the caught bug, for the record)",
