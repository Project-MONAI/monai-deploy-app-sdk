...
def run_case(name: str, in_shape, out_shape, order: int) -> int:
    ...
    import cupy as cp
    import gpu_bootstrap  # noqa: F401 (RMM first — shipping import order)
    import numpy as np
    ...
    got = np.ascontiguousarray(
        gpu_zoom_grid_mode(cp.asarray(np.ascontiguousarray(x[0])), zf, order, out_shape=new_shape).get()
    )
    if not (ref.shape == got.shape and ref.dtype == got.dtype and np.array_equal(ref, got)):
    ...
    if got_c.ndim == 4 and got_c.shape[0] == 1:  # gpu_zoom_resize adds a channel axis
        got_c = got_c[0]
    if not (ref_c.shape == got_c.shape and ref_c.dtype == got_c.dtype and np.array_equal(ref_c, got_c)):
    ...
    if failures:
        for label, r, g in failures:
            ...
            print(
                f"  [FAIL] {label}: {n}/{r.size} voxels differ " f"({100.0 * n / r.size:.6f}%), max_abs={max_abs:.6e}"
            )
        print(f"CASE {name}: FAIL ({in_shape} -> {new_shape} o{order})")
        return 1
    print(f"CASE {name}: PASS ({in_shape} -> {new_shape} o{order}) " f"byte-identical on both levels")
    return 0
...
def main() -> int:
    ...
    print(
        f"device: {os.environ.get('CUDA_VISIBLE_DEVICES', '0')} | "
    ...
    if bad:
        print(
            f"VERDICT: DISCARD — {bad} not byte-identical (D-22a: the custom "
            f"kernel is dropped from the shipping path; rewire to stock CuPy)"
        )
        return 1
    print(
        "VERDICT: KEEP — all cases byte-identical vs the scipy/skimage "
