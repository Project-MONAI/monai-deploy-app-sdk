...
MODEL_ROOT = REPO_ROOT / "examples" / "apps" / "cchmc_nnunet_fifteen_ckpt_app" / "models"
...
from postresample_operator import _determine_do_sep_z_and_axis, resample_probabilities_to_shape  # noqa: E402
from preprocess_operator import _get_bbox_from_mask  # noqa: E402
...
def compare(name: str, off, on) -> dict:
    ...
    print(
        f"  {name}: shape {off.shape} dtype {off.dtype}  "
    ...
    return {
        ...
        "dtype": str(off.dtype),
        "elements_equal_pct": eq,
        "max_abs_diff": mad,
        "meets_99pct_bar": eq >= 99.0,
    }
...
def main() -> int:
    ...
    def load_ct(dcm_dir: Path):
        ...
        pos = [np.asarray(d.ImagePositionPatient, dtype=np.float64) for d in datasets]
    ...
    def load_seg_labelmap(seg_dir: Path) -> np.ndarray:
        ...
        arr = ds.pixel_array
        assert arr.ndim == 3 and set(np.unique(arr)) <= {
            0,
            1,
    ...
    for label, params in (("fullres/cascade", p_full), ("lowres", p_low)):
        ...
        set_flag(True)
        on = _resample_to_shape(cp.asarray(d), ns, original_spacing, params.spacing, params)
        if isinstance(on, cp.ndarray):
            on = on.get()
        set_flag(False)
        results.append(compare(f"image o3 {label} {d.shape[1:]}->{tuple(ns)}", off, on))
    ...
    set_flag(False)
    results.append(compare(f"seg o1 multihot {seg_crop.shape}->{tuple(ns_full)}", off1, on1))
    results.append(compare(f"seg o0 nearest {seg_crop.shape}->{tuple(ns_full)}", off0, on0))
    ...
    seg_low = ndi_zoom(
        seg3,
        tuple(prob_shape[i] / seg3.shape[i] for i in range(3)),
        order=0,
        mode="nearest",
    ).astype(np.float32)
    prob = np.ascontiguousarray(np.stack([1.0 - seg_low, seg_low]))  # (2, *lowres) bg/fg
    ...
    set_flag(False)
    print(f"prob: (2, {prob_shape}) -> {tuple(crop_shape)} (order 1), " f"do_separate_z={do_sep}")
    results.append(compare(f"prob o1 2ch (2,{prob_shape})->{tuple(crop_shape)}", offp, onp))

    print()
    all_ok = all(r.get("meets_99pct_bar") for r in results)
    print(f"STEP 6 per-tensor bar (>=99% equal): " f"{'ALL MEET' if all_ok else 'NOT MET'}")
    if args.json:
        out = Path(args.json)
