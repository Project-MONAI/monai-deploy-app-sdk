...
from config import load_inference_params, load_preprocess_params, resolve_run_model_list  # noqa: E402

MODEL_ROOT = Path("/raid/monai-deploy-app-sdk/examples/apps/cchmc_nnunet_fifteen_ckpt_app/models")
...
def test_standalone_lowres_runs_with_self_ensemble():
    ...
    print(
        "PASS test_standalone_lowres_runs_with_self_ensemble (plan 04 extension; empty list still raises reference error)"
    )
...
def test_reference_reorder_with_reversed_pair():
    plans = load_plans()
    run, ensemble = resolve_run_model_list(["3d_cascade_fullres", "3d_lowres"], plans, MODEL_ROOT)
...
def test_seg_resample_replica():
    ...
    assert (
        params.resample_seg_order,
        params.resample_seg_order_z,
        params.resample_seg_force_separate_z,
    ) == (
        1,
        0,
        None,
...
def test_ensemble_order():
    ...
    assert torch.equal(got_reordered, got), "list-order reconstruction did not yield the ensemble-order result"
...
def test_preprocess_image_cascade_two_channel():
    ...
    assert tuple(vol.shape[1:]) == tuple(
        int(s) for s in props["new_shape"]
...
if __name__ == "__main__":
    ...
    print(
        "ALL PASS (plan 03: model-list, cascade params, seg-resample replica, one-hot, revert_crop_gpu; plan 04: ensemble order)"
    )
