...
def test_defer_forced_synthetic():
    ...
    print(
        f"PASS test_defer_forced_synthetic (total {plan.total_bytes / GB:.2f} GB " f"> 40 GB -> defer_to_incremental)"
    )
...
def test_boundary():
    ...
    assert (
        at_boundary.strategy == "full_volume"
    ), f"expected full_volume at exactly total_bytes, got {at_boundary.strategy!r}"
    assert (
        below_boundary.strategy == "defer_to_incremental"
