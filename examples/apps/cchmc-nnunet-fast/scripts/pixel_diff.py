...
def resolve_seg_file(path: str) -> Path:
    ...
    if len(dcms) > 1:
        print(
            f"pixel_diff: WARNING: {len(dcms)} .dcm files under {p}; " f"comparing {dcms[0].name}",
            file=sys.stderr,
        )
    return dcms[0]
...
def read_seg(dcm_path: Path):
    ...
    if int(ds.SamplesPerPixel) != 1:
        fail_usage(
            f"{dcm_path}: SamplesPerPixel={ds.SamplesPerPixel}; this tool " f"compares binary (1-sample) segmentations"
        )
    ...
    if bits.size != rows * cols * frames:
        ...
        if bits.size // (rows * cols) != frames:
            fail_usage(
                f"{dcm_path}: PixelData {len(raw)} bytes does not match " f"{frames}x{rows}x{cols} 1-bit geometry"
            )
...
def geometry_report(ds: "object") -> dict:
    ...
    g = {
        ...
        "ImageOrientationPatient": (
            _lst(ds.ImageOrientationPatient) if hasattr(ds, "ImageOrientationPatient") else None
        ),
        "ImagePositionPatient": (_lst(ds.ImagePositionPatient) if hasattr(ds, "ImagePositionPatient") else None),
    }
...
def compare(
    ...
    if n_diff:
        ...
        for f_ in flat:
            ...
            c = f_ % bits_a.shape[2]
            diff_coords.append([z, r, c, int(bits_a.reshape(-1)[f_]), int(bits_b.reshape(-1)[f_])])
    ...
    report = {
        "a": {
            "name": name_a,
            "file": str(fa),
            "SOPInstanceUID": str(ds_a.SOPInstanceUID),
            "voxels": n_a,
        },
        "b": {
            "name": name_b,
            "file": str(fb),
            "SOPInstanceUID": str(ds_b.SOPInstanceUID),
            "voxels": n_b,
        },
        ...
        "tolerance": {
            ...
            "max_diff_voxels": max_diff_voxels,
            "exact": exact,
        },
        "pass": passed,
    ...
    print(f"  geometry:            {'MATCH' if geo_ok else 'MISMATCH'}" + ("" if geo_ok else f"  {geo_diff}"))
    print(f"  byte-identity:       {byte_identity_pct:.5f}%  ({same_bytes}/{n} bytes)")
    print(
        f"  differing voxels:    {n_diff} / {total} " f"({100.0 * n_diff / total:.6f}%)"
        if total
        else f"  differing voxels:    {n_diff}"
    )
    print(f"  IoU:                 {iou:.6f}")
    if diff_coords:
        ...
        for d in diff_coords:
            print(f"    {d}")
    print(f"  tolerance:           min-identity {min_identity}%, " f"max-diff-voxels {max_diff_voxels}, exact={exact}")
...
def main(argv=None) -> int:
    ...
    try:
        rc, report = compare(
            Path(args.seg_a).name or "A",
            args.seg_a,
            Path(args.seg_b).name or "B",
            args.seg_b,
            args.min_identity,
            args.max_diff_voxels,
            args.exact,
