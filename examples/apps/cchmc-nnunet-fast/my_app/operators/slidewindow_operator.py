...
def load_model_bundle(
    ...
    ``use_mirroring=None`` (Phase 29, HYPER-06) defers to the plan-driven
    value ``params.use_mirroring`` (``resolve_use_mirroring`` precedence);
    an explicit True/False still wins over the plan value.
    """
    device = torch.device(device)
    params = load_inference_params(model_path, config_name, checkpoint_name)
    if use_mirroring is None:
        use_mirroring = params.use_mirroring
...
class SlideWindowOperator(Operator):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            use_mirroring: TTA mirroring. ``None`` (default, Phase 29 / HYPER-06)
                = plan-driven via ``resolve_use_mirroring`` (per-config plans
                key > top-level plans key > True); an explicit True/False
                wins over the plan value. Mirroring axes come from the
                checkpoint metadata.
    ...
    def _load_model(self) -> ModelBundle:
        ...
        # Phase 29 (HYPER-06): the gate driver greps this line VERBATIM —
        # exact format, one line, only when TTA is actually active.
        if self._bundle.use_mirroring and self._bundle.mirror_axes:
            axes = self._bundle.mirror_axes
            self._logger.info(
                "TTA on: config=%s mirror_axes=(%s) combinations=%d",
                self._bundle.config_name,
                ", ".join(map(str, axes)),
                len(build_mirror_axis_combinations(axes, 5)),
            )
        return self._bundle
