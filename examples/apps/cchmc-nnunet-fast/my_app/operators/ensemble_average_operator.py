    ...
    * a list/tuple of torch tensors or holoscan ``Tensor`` objects (DLPack),
    ...
    if isinstance(probabilities, (list, tuple)):
        ...
        try:
            ...
            raise ValueError(f"unsupported probabilities input type {type(probabilities)!r}") from e
        tensors = None
    if not isinstance(probabilities, (list, tuple)):
        if volume.ndim == 4:
            ...
            raise ValueError(
                "expected (C, Z, Y, X) or (N, C, Z, Y, X) probability tensors, " f"got ndim={volume.ndim}."
            )
    for t in tensors:
        if t.ndim != 4:
...
def average_probabilities(probabilities: Any) -> torch.Tensor:
    ...
    for t in tensors[1:]:
        if tuple(t.shape) != reference_shape:
            raise ValueError(
                f"all per-config probability volumes must share shape {reference_shape}, " f"got {tuple(t.shape)}."
            )
...
class EnsembleAverageOperator(Operator):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._config_names: Optional[List[str]] = [str(c) for c in config_names] if config_names is not None else None
        if self._config_names is not None:
            ...
            super().__init__(fragment, CountCondition(fragment, len(self._config_names)), *args, **kwargs)
        else:
        ...
        with nvtx_range("ensemble_average"):
            ...
            if self._config_names is not None:
                ...
                for cfg in self._config_names:
                    ...
                    if vol is None:
                        raise ValueError(f"EnsembleAverageOperator received no 'prob_{cfg}' input.")
                    received[cfg] = vol
            ...
            if self._defer_strategy:
                ...
                self._logger.info("ensemble_average: defer_strategy active (one-config-at-a-time accumulation)")
