...
def create_overlay(image_volume: np.ndarray, label_volume: np.ndarray, alpha: float = 0.7) -> np.ndarray:
    ...
    if image_volume.shape != label_volume.shape:
        raise ValueError(f"image and label shapes must match: {image_volume.shape} vs {label_volume.shape}.")
    ...
    image_normalized = (
        np.zeros_like(image_volume, dtype=np.float64) if denom == 0 else (image_volume - image_min) / denom
    )
