...
def resolve_use_mirroring(plans: Mapping[str, Any], config_name: str) -> bool:
    """Phase 29 (HYPER-06 enabler): resolve the TTA mirroring flag from the bundle plans.

    Precedence (29-01-PLAN interfaces — re-implementation of ts-fast's D-04
    against cchmc's diverged code; default differs ON PURPOSE):
      ...
      3. ``True`` — cchmc's shipped behavior (the 15-ckpt airway bundle sets
         no key anywhere: 0/4 configs, so every real bundle resolves to TTA
         on and this change is behavior-neutral by construction).

    Pure function: never raises on malformed/missing keys (``None`` plans,
    missing/``None`` "configurations", non-dict config entry, unknown config
    name) — those all fall through to the top-level key or the True default.
    """
    ...
    if isinstance(configurations, Mapping):
        ...
        if isinstance(cfg_entry, Mapping) and "use_mirroring" in cfg_entry:
