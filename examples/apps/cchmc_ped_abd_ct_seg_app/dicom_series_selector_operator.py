        ...
        for series in study.get_all_series():
            ...
            for key, value_to_match in attributes.items():
                ...
                if not attr_value:
                    ...
                    # CT Liver-Spleen specific case
                    # for our rules, SeriesDescription check can only rule out series that has undesired values
                    # Type 3 tag for CT modality - if tag absent, don't rule out series
                    if key == "SeriesDescription":
                        logging.warning("     Due to negative check per our rules, not ruling out series")
                        matched = True
                    else:
                        matched = False
