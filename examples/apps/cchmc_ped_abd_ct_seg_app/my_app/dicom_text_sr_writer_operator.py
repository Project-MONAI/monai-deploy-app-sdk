...
class DICOMTextSRWriterOperator(Operator):
    ...
    Named inputs:
        text: text content to be encapsulated as a Content Sequence in a DICOM instance file.
    ...
    def compute(self, op_input, op_output, context):
        ...
        # content_sequence_elements - prepare result_text for downstream Content Sequence writing
        content_sequence_elements = []

        # Helper method for parsing result_text items
        def parse_value_and_unit(text: str):
            parts = text.strip().split()
            if len(parts) != 2:
              raise ValueError(f"Invalid measurement format: {text}")
            
            value = int(parts[0])
            unit = parts[1]
            return value, unit

        # Generate payload for content sequence
        for biomarker_name, biomarker_dict in result_text.items():
            
            # Concept Name Code Sequence
            concept_name_code = Dataset()
            # "Coding scheme designators beginning with "99" and the coding scheme
            # designator "L" are defined in HL7 V2 to be private or local coding
            # schemes." Source: https://dicom.innolitics.com/ciods/basic-text-sr/
            # sr-document-content/0040a730/0040a043/00080102
            # Stephen Clark confirmed 99_BH should be used
            ...
            # Extract CodeMeaning based on unit - ml, cm^2 or biomarker
            if biomarker_name.split("_")[-1].lower() == "hu":
                ...
                code_meaning = unit  # Fallback to using the unit itself

            self._logger.info(f"Preparing Content Sequence for biomarker: {biomarker_name}, value: {value}, unit: {unit}, code_meaning: {code_meaning}")
            ...
            measured_value.NumericValue = str(value) # stringify to prevent writing as Decimal String
            ...
            if z_score is not None:
                ...
                # Measured Value Sequence
                measured_value = Dataset()
                measured_value.NumericValue = str(round(z_score,2)) # stringify to prevent writing as Decimal String
                measured_value.MeasurementUnitsCodeSequence = Sequence([])  # No units for Z-score
            ...
            if percentile is not None:
                ...
                # Measured Value Sequence
                measured_value = Dataset()
                measured_value.NumericValue = str(round(percentile,2)) # stringify to prevent writing as Decimal String
                measured_value.MeasurementUnitsCodeSequence = Sequence([measurement_units_code])  # No units for Percentile
