...
class SegmentationContourOperator(Operator):
    ...
    The operator takes a segmentation mask and a label dictionary, and produces a contour image where
    the boundaries of each labeled region are highlighted. This is useful for visualization, quality control,
    or exporting contours for further analysis or reporting.
    ...
    def __init__(self, fragment: Fragment, *args, labels_dict: Optional[dict] = None, **kwargs):
        ...
        self.input_labels = labels_dict if labels_dict is not None else {"organ1": 1}
    ...
    def create_contour(
        ...
        if isinstance(label_image, Image):
            ...
            if label_image.ndim == 3:
                ...
                raise ValueError(
                    f"Unsupported number of dimensions in label image: {label_image.ndim}  Expected 2 or 3."
                )
        ...
        result_image = self._mt_array_to_image(contour_image, metadata)

        return result_image

    def _mt_array_to_image(self, out_ndarray, input_img_metadata) -> Image:
        """
