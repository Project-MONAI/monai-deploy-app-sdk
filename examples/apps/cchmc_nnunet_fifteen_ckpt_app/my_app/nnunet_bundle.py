...
def convert_monai_bundle_to_nnunet(nnunet_config: dict, bundle_root_folder: str, fold: int = 0) -> None:
    ...
    if not os.path.exists(os.path.join(nnunet_model_folder, DATASET_JSON_FILENAME)):
        shutil.copy(f"{bundle_root_folder}/models/jsonpkls/{DATASET_JSON_FILENAME}", nnunet_model_folder)
    if not os.path.exists(os.path.join(nnunet_model_folder, PLANS_JSON_FILENAME)):
        shutil.copy(f"{bundle_root_folder}/models/jsonpkls/{PLANS_JSON_FILENAME}", nnunet_model_folder)
    ...
    if not os.path.exists(os.path.join(nnunet_model_folder, NNUNET_CHECKPOINT_FILENAME)):
        shutil.copy(f"{bundle_root_folder}/models/{NNUNET_CHECKPOINT_FILENAME}", nnunet_model_folder)
...
class ModelnnUNetWrapper(torch.nn.Module):
    ...
    def __init__(
        ...
        if dataset_json is None:
            dataset_json = load_json(join(Path(model_parent_dir), "jsonpkls", DATASET_JSON_FILENAME))
        if plans is None:
            plans = load_json(join(Path(model_parent_dir), "jsonpkls", PLANS_JSON_FILENAME))
        ...
        if nnunet_config is None:
            checkpoint_path = join(Path(model_training_output_dir), NNUNET_CHECKPOINT_FILENAME)
