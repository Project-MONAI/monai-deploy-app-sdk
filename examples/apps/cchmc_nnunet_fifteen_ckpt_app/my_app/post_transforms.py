            ...
            for label_name in self.label_names.keys():
                ...
                if label_name != "background":
                    ...
                    if hasattr(pred_mask, "affine"):
                        # voxel size
                        # Ensure the affine matrix is collapsed to shape (4, 4)
                        affine_matrix = np.squeeze(pred_mask.affine)
                        if affine_matrix.shape != (4, 4):
                            raise ValueError(f"Affine matrix must have shape (4, 4), but got {affine_matrix.shape}")

                        # Calculate voxel size
                        voxel_size = np.abs(np.linalg.det(affine_matrix[:3, :3]))
                    ...
                    pred_volumes[label_name] = int(round(label_volume_ml, 2))
            ...
            if label_image.ndim == 4:  # Check if the label image is 4D with a channel dimension
                ...
                for i in range(label_image.shape[-1]):
                    ...
                    for label in unique_labels:
                        ...
                        binary_mask = binary_mask.astype(np.float32)  # Convert to float32 for LabelToContour
                        thick_edges = LabelToContour()(binary_mask)

                        # Convert the edges back to binary mask
                        thick_edges = (thick_edges > 0).astype(np.uint8)
    ...
    def __call__(self, data):
        ...
        for organ, volume in organ_volumes.items():
            ...
            if label_index in self.output_labels:
                ...
                volume_text.append(f"{organ.capitalize()} Volume: {volume} mL")
