...
| 2.4 ~ | Assemble multi-fragment DAG: each config (2D, 3D_fullres, 3D_lowres, 3D_cascade_fullres) as independent Fragment | PIPE-03 | Operator-level support DONE in plan 03 (0b265e1): `resolve_run_model_list` with reference semantics + data-driven previous-stage auto-insertion; DAG assembly itself is plan 04 |
| 2.5 ~ | Wire cascade path: 3D_lowres segmentation → one-hot channel stack → 3D_cascade_fullres input | PIPE-04 | Operator-level plumbing DONE in plan 03 (c930280, 1d5d340): lowres_seg port contract (argmax uint8, original orientation, no CC) + 2-channel input, zero disk I/O, bit-exact vs vendored reference; the cross-fragment FLOW wiring is plan 04 |
