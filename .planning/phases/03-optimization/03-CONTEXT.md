...
- **D-22a (AMENDED 2026-08-19, user directive — supersedes D-22's byte-identity arbiter):** the custom RawKernel's in-flight test run is the **one and only remaining kernel attempt**. If it does not achieve byte-identity: ditch the custom-kernel path (kernel code stays committed as provenance but is NOT wired into the shipping flag path) and port to **stock CuPy** resampling (`cupyx.scipy.ndimage.map_coordinates`/`zoom`) at the same call sites, behind the same flag.
- **D-22b (AMENDED 2026-08-19, user directive — new accuracy gate):** the arbiter for the CuPy resampler is **≥99% accuracy vs the CPU scipy reference**, not byte-identity: (1) per resampled tensor ≥99% of elements equal to the scipy output; (2) the final 4-config gate suite with the flag ON must show SEG byte-identity ≥99.0% **and** IoU ≥0.99 **and** SR volume delta ≤0.1% vs the oracles (catches significant divergence; small tolerance is explicitly acceptable). If green, the flag ships **ON** (GPU resample becomes the default path). GPUP-01/02 then land as met-with-documented-tolerance. Rationale (user): the real requirement is a fast model that does NOT diverge significantly in predictions; ensemble averaging further absorbs bit-level noise; byte-for-byte was overly strict and cost disproportionate effort.
