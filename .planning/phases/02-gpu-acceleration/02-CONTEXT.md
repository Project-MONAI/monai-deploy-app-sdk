...
- **D-18:** Latency bar: ANY positive E2E improvement vs Phase 1's 61.8 s, reported with per-operator deltas vs Phase 1 and vs the 169.7 s reference baseline. No hard percentage target. **Refined + confirmed by user 2026-08-19:** report TWO bars — (a) same-scope fullres-only vs 61.8 s = the positive-improvement gate, (b) headline bundle vs 169.7 s — with the scope asymmetry (Phase 1's 61.8 s was fullres-only; serial 3-config bundle ≈ 70–80 s) documented as a deviation from the literal D-18 wording. See plan 06.
