...
def run_fast_app(model_list_env, out_dir: Path):
    ...
    if m:
        run_list = ast.literal_eval(m.group(1))
        ensemble_list = ast.literal_eval(m.group(2))
...
def main(argv=None) -> int:
    ...
    for row in GATE_ROWS:
        ...
        r = {"row": row["row"],
             ...
             "oracle": row["oracle"]}
        try:
            row_result = _run_gate_row(row, scratch)
        except Exception as e:  # noqa: BLE001 — a crashed row is a FAIL,
            # never abort the whole gate run
            import traceback
            row_result = {"pass": False,
                          "fail_reasons": [f"gate row crashed: {e}"],
