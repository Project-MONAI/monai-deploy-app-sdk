...
class ModelFactory:
    ...
    @staticmethod
    def create(path: Union[str, Path], name: str = "", model_type: Optional[str] = None) -> Model:
        """Creates a model object.

        Args:
            path (Union[str, Path]): A path to the model.
            name (str): A name of the model.
            model_type (Optional[str]): A type of the model.
    ...
    @staticmethod
    def detect_model_type(path: Union[str, Path], model_type: Optional[str] = None) -> Tuple[str, Type[Model]]:
        """Detects the model type based on a model path.

        Args:
            path (Union[str, Path]): A path to the model file/folder.
            model_type (Optional[str]): A model type.
